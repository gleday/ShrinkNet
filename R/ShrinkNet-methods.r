...
setMethod(
  ...
  definition = function(object, ...) {
    ...
    plot(
      2:length(xx), xx[-1],
      xlab = "iteration",
      ylab = "average variational lower bound"
    )
  }
)
...
setMethod(
  ...
  definition = function(object, ...) {
    ...
    plot(
      xx,
      dgamma(xx, shape = shapes[pp], rate = rates[pp]),
      xlim = c(x1, x2),
      type = "l",
      xlab = expression(tau[j]^-2),
      ylab = "probability",
      ...
    )
  }
...
setMethod(
  ...
  definition = function(object) {
    ...
    mat$"index1" <- indx[, 1]
    mat$"index2" <- indx[, 2]
    mat$score <- apply(indx, 1, function(x) {
      object@kappa[x[1], x[2]]
    }
    )
    mat$logMaxBF <- apply(indx, 1, function(x) {
      object@logMaxBFs[x[1], x[2]]
    }
    )
    mat$blfdr <- apply(
      indx,
      ...
      function(x) {
        object@p0 /
          (exp(object@logMaxBFs)[x[1], x[2]] * (1 - object@p0) + object@p0)
      }
    )
...
#' @rdname ShrinkNet-class
#' @aliases topEdges
#' @param nb maximum number of edges or nodes (depending on
#' the function it applies to) to return. Default is 20.
setMethod(
  f = "topEdges",
  signature = "ShrinkNet",
  definition = function(object, nb = 20) {
    themat <- object@kappa
    vals <- sort(
      unique(themat[upper.tri(themat, diag = FALSE)]),
      decreasing = TRUE
    )
    ...
    labs <- rownames(myadj)
    myscores <- apply(indx, 1, function(x) {
      themat[x[1], x[2]]
    })
    nodes <- t(apply(indx, 1, function(x) {
      c(labs[x[1]], labs[x[2]])
    }))
    selected <- apply(indx, 1, function(x) {
      myadj[x[1], x[2]]
    })
    mat <- data.frame(
      nodes, indx, myscores, selected,
      stringsAsFactors = FALSE
    )
    colnames(mat) <- c(
      "node1", "node2", "index1",
      "index2", "score", "selected"
    )
    mat <- mat[order(mat$score, decreasing = TRUE), ]
    rownames(mat) <- NULL
    mat
